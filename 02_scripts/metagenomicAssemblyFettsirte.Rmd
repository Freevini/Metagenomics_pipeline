...
```{bash file shortcuts,eval=FALSE}
...
```{r packages, warning=FALSE, echo=FALSE,eval=FALSE}
...
source /home/bioinf/bioinf_archive/43_sovi_storage/Projects/Fettsirte/Run20171109/02_scripts/fileShortcuts.sh
###-----------------
##blast rawread flye assembly (entire procaryotic ncbi database)
###-----------------

for name in cottens lapraz
   do
     blastn -num_threads 2 -max_hsps 1 -max_target_seqs 1 -task megablast -show_gis -query $DATA_ROOT/15_flye/assembly_${name}_raw/fasta/contigs.fasta -outfmt "6 qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore stitle slen" -db $blastdb/all_prokaryotes -out $DATA_ROOT/15_flye/assembly_${name}_raw/blast/blastAssemblyaginstContigs.txt -evalue 0.01 -word_size 64
   done
...
for name in cottens lapraz
   do
     blastn -num_threads 2 -max_hsps 1 -max_target_seqs 1 -task megablast -show_gis -query $DATA_ROOT/15_flye/assembly_${name}_min5000/fasta/scaffolds.fasta -outfmt "6 qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore stitle slen" -db $blastdb/all_prokaryotes -out $DATA_ROOT/15_flye/assembly_${name}_min5000/blast/blastAssemblyaginstContigs.txt -evalue 0.01 -word_size 64
   done
...
for name in cottens lapraz
   do
     blastn -num_threads 2 -max_hsps 1 -max_target_seqs 1 -task megablast -show_gis -query $DATA_ROOT/15_flye/assembly_${name}_raw/fasta/scaffolds.fasta -outfmt "6 qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore stitle slen" -db $blastdb/all_prokaryotes -out $DATA_ROOT/15_flye/assembly_${name}_raw/blast/blastAssemblyaginstScaffolds.txt -evalue 0.01 -word_size 64
   done
...
for name in cottens lapraz
   do
     ${minimap2_root} -t 2 -ax map-pb $DATA_ROOT/15_flye/assembly_${name}_raw/fasta/contigs.fasta $DATA_ROOT/01_SMRTlink_data/${name}/reads_${name}_raw.fastq |samtools sort -@8 -O BAM -o $DATA_ROOT/15_flye/assembly_${name}_raw/rawReads2assembly_minimap2/raw_${name}_minimap2Mapped_sorted.bam - 
   
   done 

##-------   
##min5000bp mappingminimap2 to rawRead assembly
##-------   
for name in cottens lapraz
   do
     ${minimap2_root} -t 2 -ax map-pb $DATA_ROOT/15_flye/assembly_${name}_min5000/fasta/contigs.fasta $DATA_ROOT/01_SMRTlink_data/${name}/reads_${name}_raw.fastq |samtools sort -@8 -O BAM -o $DATA_ROOT/15_flye/assembly_${name}_min5000/rawReads2assembly_minimap2/raw_${name}_minimap2Mapped_sorted.bam - 
...
for name in cottens lapraz
   do
     ${minimap2_root} -t 12 -ax map-pb $DATA_ROOT/15_flye/assembly_${name}_raw/fasta/scaffolds.fasta $DATA_ROOT/01_SMRTlink_data/${name}/reads_${name}_raw.fastq |samtools sort -@8 -O BAM -o $DATA_ROOT/15_flye/assembly_${name}_raw/rawReads2assembly_minimap2/raw_${name}_minimap2Mapped_Scaffolds_sorted.bam - 
...
for name in cottens lapraz
   do
     ${minimap2_root} -t 12 -ax map-pb $DATA_ROOT/15_flye/assembly_${name}_min5000/fasta/scaffolds.fasta $DATA_ROOT/01_SMRTlink_data/${name}/reads_${name}_raw.fastq |samtools sort -@8 -O BAM -o $DATA_ROOT/15_flye/assembly_${name}_min5000/rawReads2assembly_minimap2/raw_${name}_minimap2Mapped_Scaffolds_sorted.bam - 
...
##-------   
##bamqc mappingminimap2 to rawRead assembly (contigs)
##-------   
for name in cottens lapraz
   do
      mkdir -p $DATA_ROOT/15_flye/assembly_${name}_raw/rawReads2assembly_minimap2/bamqc

      samtools view -b -f 4 $DATA_ROOT/15_flye/assembly_${name}_raw/rawReads2assembly_minimap2/raw_${name}_minimap2Mapped_sorted.bam > $DATA_ROOT/15_flye/assembly_${name}_raw/rawReads2assembly_minimap2/raw_${name}_minimap2Mapped_sorted_unampped.bam
     ...
     # bedtools bamtofastq -i $DATA_ROOT/05_abruijn/${date}assembly_${name}/reads2assembly_align/aligned_sorted_unmapped.bam -fq $DATA_ROOT/05_abruijn/${date}assembly_${name}/reads2assembly_align/aligned_sorted_unmapped.fastq
      ...
      $QUAL_ROOT/qualimap bamqc -bam $DATA_ROOT/15_flye/assembly_${name}_raw/rawReads2assembly_minimap2/raw_${name}_minimap2Mapped_sorted.bam -outdir $DATA_ROOT/15_flye/assembly_${name}_raw/rawReads2assembly_minimap2/bamqc --java-mem-size=2G
      
      grep "Globals" -A 12  $DATA_ROOT/15_flye/assembly_${name}_raw/rawReads2assembly_minimap2/bamqc/genome_results.txt |tail -11|sed '/^\s*$/d'|sed 's/=/\t/g'|sed -e 's/(.*%)//'|sed 's/,//g' > $DATA_ROOT/15_flye/assembly_${name}_raw/rawReads2assembly_minimap2/bamqc/global_stats.txt
      
      grep "Coverage per contig" -A 1000  $DATA_ROOT/15_flye/assembly_${name}_raw/rawReads2assembly_minimap2/bamqc/genome_results.txt > $DATA_ROOT/15_flye/assembly_${name}_raw/rawReads2assembly_minimap2/bamqc/contig_stats.txt
 
      samtools index $DATA_ROOT/15_flye/assembly_${name}_raw/rawReads2assembly_minimap2/raw_${name}_minimap2Mapped_sorted.bam
  done
  

##-------   
##bamqc mappingminimap2 to min500bp assembly (contigs)
##-------   
for name in cottens lapraz
   do
      mkdir -p $DATA_ROOT/15_flye/assembly_${name}_min5000/rawReads2assembly_minimap2/bamqc

      samtools view -b -f 4 $DATA_ROOT/15_flye/assembly_${name}_min5000/rawReads2assembly_minimap2/raw_${name}_minimap2Mapped_sorted.bam > $DATA_ROOT/15_flye/assembly_${name}_min5000/rawReads2assembly_minimap2/raw_${name}_minimap2Mapped_sorted_unampped.bam
      
     # bedtools bamtofastq -i $DATA_ROOT/05_abruijn/${date}assembly_${name}/reads2assembly_align/aligned_sorted_unmapped.bam -fq $DATA_ROOT/05_abruijn/${date}assembly_${name}/reads2assembly_align/aligned_sorted_unmapped.fastq
      
      $QUAL_ROOT/qualimap bamqc -bam $DATA_ROOT/15_flye/assembly_${name}_min5000/rawReads2assembly_minimap2/raw_${name}_minimap2Mapped_sorted.bam -outdir $DATA_ROOT/15_flye/assembly_${name}_min5000/rawReads2assembly_minimap2/bamqc --java-mem-size=2G
      
      grep "Globals" -A 12  $DATA_ROOT/15_flye/assembly_${name}_min5000/rawReads2assembly_minimap2/bamqc/genome_results.txt |tail -11|sed '/^\s*$/d'|sed 's/=/\t/g'|sed -e 's/(.*%)//'|sed 's/,//g' > $DATA_ROOT/15_flye/assembly_${name}_min5000/rawReads2assembly_minimap2/bamqc/global_stats.txt
      
      grep "Coverage per contig" -A 1000  $DATA_ROOT/15_flye/assembly_${name}_min5000/rawReads2assembly_minimap2/bamqc/genome_results.txt > $DATA_ROOT/15_flye/assembly_${name}_min5000/rawReads2assembly_minimap2/bamqc/contig_stats.txt
 
      samtools index $DATA_ROOT/15_flye/assembly_${name}_min5000/rawReads2assembly_minimap2/raw_${name}_minimap2Mapped_sorted.bam
  done
...
for name in cottens lapraz
   do
      mkdir -p $DATA_ROOT/15_flye/assembly_${name}_raw/rawReads2assembly_minimap2/bamqc_scaffolds

      samtools view -b -f 4 $DATA_ROOT/15_flye/assembly_${name}_raw/rawReads2assembly_minimap2/raw_${name}_minimap2Mapped_Scaffolds_sorted.bam > $DATA_ROOT/15_flye/assembly_${name}_raw/rawReads2assembly_minimap2/raw_${name}_minimap2Mapped_sorted_Scaffolds_unampped.bam
      
     # bedtools bamtofastq -i $DATA_ROOT/05_abruijn/${date}assembly_${name}/reads2assembly_align/aligned_sorted_unmapped.bam -fq $DATA_ROOT/05_abruijn/${date}assembly_${name}/reads2assembly_align/aligned_sorted_unmapped.fastq
      
      $QUAL_ROOT/qualimap bamqc -bam $DATA_ROOT/15_flye/assembly_${name}_raw/rawReads2assembly_minimap2/raw_${name}_minimap2Mapped_Scaffolds_sorted.bam -outdir $DATA_ROOT/15_flye/assembly_${name}_raw/rawReads2assembly_minimap2/bamqc_scaffolds --java-mem-size=2G
      
      grep "Globals" -A 12  $DATA_ROOT/15_flye/assembly_${name}_raw/rawReads2assembly_minimap2/bamqc_scaffolds/genome_results.txt |tail -11|sed '/^\s*$/d'|sed 's/=/\t/g'|sed -e 's/(.*%)//'|sed 's/,//g' > $DATA_ROOT/15_flye/assembly_${name}_raw/rawReads2assembly_minimap2/bamqc_scaffolds/global_stats.txt
      
      grep "Coverage per contig" -A 1000  $DATA_ROOT/15_flye/assembly_${name}_raw/rawReads2assembly_minimap2/bamqc/genome_results.txt > $DATA_ROOT/15_flye/assembly_${name}_raw/rawReads2assembly_minimap2/bamqc/contig_stats.txt
 
      samtools index $DATA_ROOT/15_flye/assembly_${name}_raw/rawReads2assembly_minimap2/raw_${name}_minimap2Mapped_Scaffolds_sorted.bam
  done  
...
for name in cottens lapraz
   do
      mkdir -p $DATA_ROOT/15_flye/assembly_${name}_min5000/rawReads2assembly_minimap2/bamqc_scaffolds

      samtools view -b -f 4 $DATA_ROOT/15_flye/assembly_${name}_min5000/rawReads2assembly_minimap2/raw_${name}_minimap2Mapped_Scaffolds_sorted.bam > $DATA_ROOT/15_flye/assembly_${name}_min5000/rawReads2assembly_minimap2/raw_${name}_minimap2Mapped_sorted_Scaffolds_unampped.bam
      
     # bedtools bamtofastq -i $DATA_ROOT/05_abruijn/${date}assembly_${name}/reads2assembly_align/aligned_sorted_unmapped.bam -fq $DATA_ROOT/05_abruijn/${date}assembly_${name}/reads2assembly_align/aligned_sorted_unmapped.fastq
      
      $QUAL_ROOT/qualimap bamqc -bam $DATA_ROOT/15_flye/assembly_${name}_min5000/rawReads2assembly_minimap2/raw_${name}_minimap2Mapped_Scaffolds_sorted.bam -outdir $DATA_ROOT/15_flye/assembly_${name}_min5000/rawReads2assembly_minimap2/bamqc_scaffolds --java-mem-size=2G
      
      grep "Globals" -A 12  $DATA_ROOT/15_flye/assembly_${name}_min5000/rawReads2assembly_minimap2/bamqc_scaffolds/genome_results.txt |tail -11|sed '/^\s*$/d'|sed 's/=/\t/g'|sed -e 's/(.*%)//'|sed 's/,//g' > $DATA_ROOT/15_flye/assembly_${name}_min5000/rawReads2assembly_minimap2/bamqc_scaffolds/global_stats.txt
      
      grep "Coverage per contig" -A 1000  $DATA_ROOT/15_flye/assembly_${name}_min5000/rawReads2assembly_minimap2/bamqc/genome_results.txt > $DATA_ROOT/15_flye/assembly_${name}_min5000/rawReads2assembly_minimap2/bamqc/contig_stats.txt
 
      samtools index $DATA_ROOT/15_flye/assembly_${name}_min5000/rawReads2assembly_minimap2/raw_${name}_minimap2Mapped_Scaffolds_sorted.bam
  done   
```
####ngmlr/sniffles
...
for name in cottens lapraz
   do
    ...
    $ngmlr_root/ngmlr -t 12 -r $DATA_ROOT/15_flye/assembly_${name}_raw/fasta/scaffolds.fasta -q $DATA_ROOT/01_SMRTlink_data/${name}/reads_${name}_raw.fastq |samtools sort -@8 -O BAM -o $DATA_ROOT/15_flye/assembly_${name}_raw/rawReads2assembly_ngmlr/raw_${name}_ngmlrMapped_Scaffolds_sorted.bam - 
  done
...
for name in cottens lapraz
   do
    ...
    $ngmlr_root/ngmlr -t 12 -r $DATA_ROOT/15_flye/assembly_${name}_min5000/fasta/scaffolds.fasta -q $DATA_ROOT/01_SMRTlink_data/${name}/reads_${name}_raw.fastq |samtools sort -@8 -O BAM -o $DATA_ROOT/15_flye/assembly_${name}_min5000/rawReads2assembly_ngmlr/raw_${name}_ngmlrMapped_Scaffolds_sorted.bam - 
  done
...
for name in cottens lapraz
   do
    mkdir -p $DATA_ROOT/15_flye/assembly_${name}_raw/rawReads2assembly_ngmlr/sniffles
    $sniffles_root/sniffles -t 10 -m $DATA_ROOT/15_flye/assembly_${name}_raw/rawReads2assembly_ngmlr/raw_${name}_ngmlrMapped_Scaffolds_sorted.bam -v $DATA_ROOT/15_flye/assembly_${name}_raw/rawReads2assembly_ngmlr/sniffles/sniffles_${name}_raw.vcf
...
for name in cottens lapraz
   do
    mkdir -p $DATA_ROOT/15_flye/assembly_${name}_raw_min5000/rawReads2assembly_ngmlr/sniffles
    $sniffles_root/sniffles -t 10 -m $DATA_ROOT/15_flye/assembly_${name}_raw_min5000/rawReads2assembly_ngmlr/raw_${name}_ngmlrMapped_Scaffolds_sorted.bam -v $DATA_ROOT/15_flye/assembly_${name}_raw_min5000/rawReads2assembly_ngmlr/sniffles/sniffles_${name}_raw_min5000.vcf
